...
main_root:divider("스노맨 텔레포트")

main_root:action("모든 눈사람을 자동으로 폭발시킵니다.", {"autoexpsnowmen"}, "자동으로 모든 위치로 순간이동하여 눈사람을 폭발시킵니다. 몇 초 정도 걸립니다. (~90)", function()
...
local manually_teleport = main_root:list("눈사람에게 순간이동")

auto_exp_toggle= true
auto_exp_toggle = manually_teleport:toggle("텔레포트 자동 폭발 스노우맨", {"autoexplode"}, "눈사람에게 순간이동한 후 폭발시킵니다.", function(toggle)
    if toggle then
        explode_snowmen = true
...
for idx, coords in snowmen do
    manually_teleport:action("눈사람 #" .. idx, {}, "눈사람에게 순간이동 #".. idx, function()       
        menu.trigger_command(god_path, "on")
        ENTITY.SET_ENTITY_COORDS(players.user_ped(), coords[1], coords[2], coords[3])
        util.yield(1590)
